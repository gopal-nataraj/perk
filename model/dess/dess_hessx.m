  function [sp_hess, sm_hess] = dess_hessx(M0, T1, T2, flip, TR, TEp, TEm, varargin)
%|function [sp_hess, sm_hess] = dess_hessx(M0, T1, T2, flip, TR, TEp, TEm, varargin)
%|
%|  dual-echo steady state signal hessian w.r.t. [M0 T1 T2]
...
%|
%|  outputs
%|    sp_hess   [(odims) L L]   dess 'defocused' signal gradient at t=TEp after RF
%|    sm_hess   [(odims) L L]   dess 'refocused' signal gradient at t=TEm before RF
%|
...
if arg.mag
  sp_hess = dess_echo1_hessx_abs_gen(...
    M0, T1, T2, kap, R2p, flip, TR, TEp);
  sm_hess = dess_echo2_hessx_abs_gen(...
    M0, T1, T2, kap, R2p, flip, TR, TEm);
else
  sp_hess = dess_echo1_hessx_cmplx_gen(...
    M0, T1, T2, kap, dw, R2p, flip, TR, TEp);
  sm_hess = dess_echo2_hessx_cmplx_gen(...
    M0, T1, T2, kap, dw, R2p, flip, TR, TEm);
end

% protect against unsafe division (0/0 set to 0)
sp_hess(isnan(sp_hess)) = 0;
sm_hess(isnan(sm_hess)) = 0;

% set infinities to zero for safety
sp_hess(isinf(sp_hess)) = 0;
sm_hess(isinf(sm_hess)) = 0;

% embed back into original array size
sp_hess = embed(sp_hess, arg.mask);                                       % [(odims) L L]
sm_hess = embed(sm_hess, arg.mask);                                       % [(odims) L L]
end
