...
% hessian of received signals w.r.t. indepedent variables, x
s1_TI_TE_xy_hessx = simplify(hessian(s1_TI_TE_xy, [M0 T1 T2 inveff]));        % [L L]
s1_TI_TE_xy_hessx = reshape(s1_TI_TE_xy_hessx, [1 4 4]);                      % [1 L L]
s2_TI_TE_xy_hessx = simplify(hessian(s2_TI_TE_xy, [M0 T1 T2 inveff]));        % [L L]
s2_TI_TE_xy_hessx = reshape(s2_TI_TE_xy_hessx, [1 4 4]);                      % [1 L L]

% generate hessian function
if ~exist('IR_hessx_gen.m', 'file')
  tmp = matlabFunction(s1_TI_TE_xy_hessx, s2_TI_TE_xy_hessx,...
    'file', 'IR_hessx_gen.m',...
    'vars', [M0 T1 T2 TR TI TE flip_inv flip_ex flip_ref inveff kap dw]);
end
