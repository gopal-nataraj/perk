...
%|
%| Copyright 2005-06-18, Jeff Fessler,  University of Michigan
%| Modified  2016-04-06, Gopal Nataraj, University of Michigan
...
switch length(idot)
    ...
    case 2
        arg1 = arg([1:(idot(1)-1)]);
        arg2 = arg([(idot(1)+1):(idot(2)-1)]);
        arg3 = arg([(idot(2)+1):end]);
        if ~isfield(opt, arg1)
            fail('unknown option1 name "%s"', arg1);
        end
        if ~isfield(opt.(arg1), arg2)
        ...
        if ~isfield(opt.(arg1).(arg2), arg3)
            fail('unknown option3 name "%s"', arg3);
        end
        s = struct('type', {'.', '.', '.'},...
            'subs', {arg1, arg2, arg3});
        try
